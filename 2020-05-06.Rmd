...
#### **固定字元/組**：單寫一個字組或字元，表示那個strip固定不動只有一個出象，例如：`2`表示只有“2”一個出象，`櫻桃`只有“櫻桃”一個出象。
```{r}
...
year2 <- 
  str_extract(
    conxiDate,
    regex_pattern2)
westernYear2 <- as.integer(year2)+1639
str_replace(
  conxiDate,
  regex_pattern2,
  as.character(westernYear2)
) -> conxiDate_reformat
print(conxiDate_reformat)
dateFinal2 <- lubridate::ymd(conxiDate_reformat)
class(dateFinal2)
```

##### 之前我們使用`str_extract`函數粹取出以下地址的“xx號”包含“號”字，如果不要取到「號」字只取出數字，你會怎麼做？
```{r}
addresses <- c(
  '臺中市后里區公館里尾社路385號',
  '新竹市東區科學園路125號1樓'
