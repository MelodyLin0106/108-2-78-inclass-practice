...
library(stringr)
if(
  str_detect(studentId,'^[43](1[01][0-9]|9[0-9])[0-9]{5}$',negate=T) # 要加上"^"和"$"才能規定它只包含這串數字，不能有其他數字
) {
  warning("你所輸入的學號不正確")
}
```
...
##### 我們可以把它形成一個`convert_TaiwanDate2WesternDate()`函數：
```{r}
...
# as.logical會將0轉為False，非0的數字會轉為True
myFun(54)
myFun(33)
```
